...
Support prioritized AI coding agents (claude, gemini, qwen, codex-cli) with dedicated templates from local repo folders (templates/, templates/commands/, scripts/, memory/). Agent-specific prompts must be generated for /gmkit commands, with fallback for discontinued tools. When executing implementation tasks (e.g., /implement), agents MUST read and follow all coding standards, naming conventions, and guidelines defined in AGENTS.md before writing any code or tests. Features that integrate LLM calls at runtime MUST use a provider-agnostic abstraction so that any supported agent/model can serve as the backend; provider selection is configured at runtime. Supported models are assumed to have equivalent multimodal capabilities (e.g., vision, structured output).
...
**Version**: 1.4 | **Ratified**: 2025-12-19 | **Last Amended**: 2026-02-17
